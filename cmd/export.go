...
// HTML diagram. The output file is fully self-contained and works offline.
func runExport(args []string) error {
	fs := flag.NewFlagSet("export", flag.ExitOnError)
	fs.StringVar(&exportPlan, "plan", "", "Path to terraform show -json plan output")
	fs.StringVar(&exportState, "state", "", "Path to terraform.tfstate file")
	fs.StringVar(&exportOut, "out", "diagram.html", "Output HTML file path")
	fs.StringVar(&exportDiff, "diff", "",
		"Base plan/state to diff against — auto-detects format (enables diff mode)")
	fs.BoolVar(&exportThreat, "threat", false,
		"Run threat modelling analysis and overlay findings on the diagram")
	fs.StringVar(&exportCost, "cost", "",
		"Cost overlay: path to Infracost JSON file, or Terraform directory to auto-run infracost")
	fs.StringVar(&exportDrift, "drift", "",
		"Drift detection: refresh-only plan JSON file, or Terraform directory to auto-run terraform plan -refresh-only")
	fs.StringVar(&exportFormat, "format", "html",
		"Output format: html (default), json, or threats")
	fs.BoolVar(&exportFlow, "flow", false,
	...
	default: // "html"
		...
		if err := renderer.ExportHTML(f, g); err != nil {
